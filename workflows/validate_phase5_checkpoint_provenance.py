...
REQUIRED_PHASE5_SUMMARY_FIELDS = (
    "checkpoint_found",
    "selected_checkpoint",
    "selected_rule",
...
EXPECTED_VALUES = {
    ...
    "ppo_evidence_level": PHASE5_EVIDENCE_LEVEL,
    "checkpoint_provenance": PHASE5_CHECKPOINT_PROVENANCE,
...
def _require_fields(summary: dict[str, Any]) -> None:
    missing = [field for field in REQUIRED_PHASE5_SUMMARY_FIELDS if field not in summary]
    if missing:
        raise ValueError(f"Missing Phase 5 provenance summary fields: {missing}")
...
def _require_checkpoint_match(checkpoint_path: Path, summary: dict[str, Any]) -> None:
    ...
    if selected_resolved != checkpoint_path:
        raise ValueError(
            f"selected_checkpoint must match checkpoint path: {selected_resolved} != {checkpoint_path}"
        )
    if not summary["checkpoint_found"]:
        raise ValueError("checkpoint_found must be true for Phase 5 provenance")


def _require_numeric_health_fields(summary: dict[str, Any]) -> None:
    ...
    goal_match_error = float(summary["base_reference_goal_match_max_error"])
...
def validate_phase5_checkpoint_provenance(
    ...
    _require_numeric_health_fields(summary)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate Phase 5 PPO checkpoint provenance.")
    parser.add_argument("--checkpoint", required=True, help="Checkpoint to validate.")
    parser.add_argument(
        "--source_training_summary_path",
        required=True,
        help="Phase 5 training summary JSON proving checkpoint provenance.",
    )
    parser.add_argument("--json", action="store_true", help="Print machine-readable JSON.")
    args = parser.parse_args(argv)

    try:
        result = validate_phase5_checkpoint_provenance(args.checkpoint, args.source_training_summary_path)
    ...
    if args.json:
        ...
        print(f"source_result_bucket={result['source_result_bucket']}")
        print(f"source_controller_path={result['source_controller_path']}")
        print(f"source_adapter_mode={result['source_adapter_mode']}")
    return 0
