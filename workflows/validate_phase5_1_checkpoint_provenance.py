...
REQUIRED_PHASE5_1_SUMMARY_FIELDS = (
    "checkpoint_found",
    "selected_checkpoint",
    "selected_rule",
...
def _require_fields(summary: dict[str, Any]) -> None:
    missing = [field for field in REQUIRED_PHASE5_1_SUMMARY_FIELDS if field not in summary]
    if missing:
        raise ValueError(f"Missing Phase 5.1 provenance summary fields: {missing}")


def _require_expected_values(summary: dict[str, Any], expected_evidence_level: str) -> None:
    if expected_evidence_level == PHASE5_1_MATCHED_EVIDENCE_LEVEL:
        expected_evidence_level = PHASE5_1_CANDIDATE_EVIDENCE_LEVEL
    if expected_evidence_level not in PHASE5_1_PROVENANCE_BY_EVIDENCE_LEVEL:
        raise ValueError(f"Unsupported Phase 5.1 expected evidence level: {expected_evidence_level!r}")

    expected_values = {
        "result_bucket": PHASE5_RESULT_BUCKET,
        "checkpoint_provenance": PHASE5_1_PROVENANCE_BY_EVIDENCE_LEVEL[expected_evidence_level],
...
def _require_checkpoint_match(checkpoint_path: Path, summary: dict[str, Any]) -> None:
    ...
    if summary["checkpoint_found"] is not True:
        raise ValueError("checkpoint_found must be true for Phase 5.1 provenance")
...
def validate_phase5_1_checkpoint_provenance(
    checkpoint_path: str | Path,
    source_training_summary_path: str | Path,
    *,
    expected_evidence_level: str = PHASE5_1_CANDIDATE_EVIDENCE_LEVEL,
) -> dict[str, Any]:
    if expected_evidence_level not in PHASE5_1_EVIDENCE_LEVELS:
        raise ValueError(f"expected_evidence_level must be one of {list(PHASE5_1_EVIDENCE_LEVELS)}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate Phase 5.1 PPO checkpoint provenance.")
    parser.add_argument("--checkpoint", required=True, help="Checkpoint to validate.")
    parser.add_argument("--source_training_summary_path", required=True, help="Phase 5.1 summary JSON.")
    parser.add_argument(
        "--expected_evidence_level",
        choices=PHASE5_1_EVIDENCE_LEVELS,
        default=PHASE5_1_CANDIDATE_EVIDENCE_LEVEL,
        help="Expected Phase 5.1 evidence level.",
    )
    ...
    if args.json:
        ...
        print(f"source_ppo_evidence_level={result['source_ppo_evidence_level']}")
    return 0
