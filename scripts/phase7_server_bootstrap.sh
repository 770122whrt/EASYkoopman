...
readonly BUNDLE_PATH="/root/EasyUUV-phase7-v2.bundle"
readonly EXPECTED_COMMIT_FILE="/root/expected-source-commit.txt"
readonly TARGET_ROOT="/root/EASYkoopman-phase7-v2"
...
verify_repository="$(mktemp -d "${TMPDIR:-/tmp}/phase7-bundle-verify.XXXXXX")"
...
server_status="$(git -C "$TARGET_ROOT" -c core.excludesFile= status --porcelain=v1 --untracked-files=all)"
[[ -z "$server_status" ]] || die "server_clone_not_clean"

exec bash "$TARGET_ROOT/scripts/phase7_server_smoke.sh" "$EXPECTED_COMMIT_FILE"
