...
"$ISAACLAB_PY" -p workflows/build_phase8_main_envelope.py --root "$RESULT_ROOT" --source-commit "$source_commit" --experiment-id phase8-main-identification-v1-proposal --role-protocol "$RESULT_ROOT/main_role_assignment_protocol.json" --analysis-policy "$RESULT_ROOT/analysis_policy.json" --runtime "$runtime_json" --inventory "$RESULT_ROOT/dataset_inventory.json" --split "$RESULT_ROOT/loco_split_manifest.json" --output "$RESULT_ROOT/dataset_envelope.json"
