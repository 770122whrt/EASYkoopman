...
class HydrodynamicForceModels:
  ...
  def _calculate_inferred_half_dimensions(self, inertias, masses):
    ...
    """
    # Independent inertia scaling can violate the equivalent-box inertia constraints.
    # Clamp the inferred side-length radicand to keep drag computation numerically stable.
    radicand = (3 / (2 * masses.repeat(1, 3))) * (torch.roll(inertias, 1, 1) + torch.roll(inertias, -1, 1) - inertias)
    r = torch.sqrt(torch.clamp_min(radicand, 1.0e-9))
    return r
  ...
  def calculate_density_and_viscosity_forces(self, 
                                             ...
                                             water_rho: float,
                                             masses: torch.tensor,
                                             fluid_vel_w: torch.Tensor | None = None
                                             ):

    root_quats_b = quat_conjugate(root_quats_w)

    if fluid_vel_w is None:
      fluid_vel_w = torch.zeros_like(root_linvels_w)
    else:
      fluid_vel_w = fluid_vel_w.to(device=root_linvels_w.device, dtype=root_linvels_w.dtype)
      if fluid_vel_w.ndim == 1:
        fluid_vel_w = fluid_vel_w.unsqueeze(0).expand_as(root_linvels_w)
      elif fluid_vel_w.shape != root_linvels_w.shape:
        fluid_vel_w = fluid_vel_w.expand_as(root_linvels_w)

    rel_linvels_w = root_linvels_w - fluid_vel_w
    rel_linvels_b = quat_apply(root_quats_b, rel_linvels_w)
    root_angvels_b = quat_apply(root_quats_b, root_angvels_w)
  
    f_d, g_d = self.calculate_quadratic_drag_forces(rel_linvels_b, root_angvels_b, inertias, masses, water_rho)
    f_v, g_v = self.calculate_linear_viscous_forces(rel_linvels_b, root_angvels_b, inertias, masses, water_beta)
